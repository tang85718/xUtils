...
public class DbModelSelector {
	...
	protected DbModelSelector(Selector selector, String groupByColumnName) {
		...
		this.selector.groupBy(groupByColumnName);
	...
	public String toString() {
		...
		selector.buildFrom(result);
		selector.buildGroupBy(result);
		selector.buildOrderBy(result);
		selector.buildLimit(result);
		...
		// }
		//
		// if (selector.groupByList != null) {
		// result.append(" GROUP BY ");
		// for (int i = 0; i < selector.groupByList.size(); i++) {
		// result.append(selector.groupByList.get(i)).append(",");
		// }
		// result.deleteCharAt(result.length() - 1);
		// }
		//
		...
		// result.append(selector.orderByList.get(i)).append(",");
		// }
		// result.deleteCharAt(result.length() - 1);
		// }
		//
